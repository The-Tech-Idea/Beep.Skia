...
- **Ninth documentation round (rendered images)** — the site had no images at all, which is a real gap for a visual framework. Added `Beep.Skia.Tests/DocumentationImageGenerator.cs`, which renders every registered component type into a labelled grid per family **through the real pipeline** (`DrawingManager.Draw` on a Skia surface) and writes `Help/assets/families/*.png` (16 families plus a UI-controls grid). The generator is opt-in (`BEEP_SKIA_GENERATE_DOC_IMAGES=1`) so normal test runs and CI never write to the repository. The images are embedded at the top of every family page and on the landing page, and the docs index explains how to regenerate them. Link checker now covers 495 references, all resolving.
- **Eighth documentation round (XML docs in source, 60% -> 97%)** — the largest remaining gap was the source itself: IntelliSense showed nothing for most of the family API (FlowChart 13%, ERD 17%, ETL 18%, Business 20%, WinForms 21% documented). A new tool (`docs/_gen_xml_docs.ps1`) adds XML summaries to undocumented public members, harvesting the real description text from each member's own `ParameterInfo.Description`/`SetProp` argument where present and deriving a name-based summary otherwise; it inserts comments before attribute blocks (a first pass after attributes produced CS1587, which the tool now avoids). Result: **4221 of 4367 public members documented (97%)**, up from roughly 60%, with per-project coverage at 90-100% except `Beep.Skia.Model` at 99%. The generated Help API reference pages inherit the summaries, so the ETL reference went from mostly empty descriptions to 0 empty of 133 members and the core reference to 35 empty of 2035. Build stays warning-free (the earlier CS1570/1572/1573 fixes plus the placement fix); 501/501 tests pass.
- **Seventh documentation round (family features + remaining depth)** — the family pages were catalogs that omitted each family's analysis tooling. Added: flowchart code generation (`FlowchartCodeGenerator`, `CodeLanguage`) and simulation (`FlowchartSimulator` with `Step`/`Run`/`Trace`/`BranchSelector`), security STRIDE analysis (`StrideAnalyzer` threat results with MITRE techniques and DREAD), DREAD scoring (`DreadScore`/`DreadCalculator`) and the MITRE ATT&amp;CK catalog, state-machine transition semantics (`TriggerEvent`/`GuardCondition`/`TransitionAction` plus `StateNode` entry/exit/do behaviour), the ECAD electrical rules check (`ElectricalRulesChecker`, violations with severity, category and fix suggestion), BPMN import/export (`BpmnExporter`, `BpmnImporter`, `BpmnImportResult`) and mind-map collapse/expand (`MindMapVisibility.Apply`). Also deepened the last thin pages: `architecture/events.html` (raising semantics and an argument-selection table), `ecosystem/extensions.html` (lifecycle, packaging, checklist) and `ui-components/editors.html` (usage, save/cancel events, `SkiaComponentGrid`). Every page is now at least ~370 words; no stubs remain.
